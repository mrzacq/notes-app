# Tugas Lanjutan: CI/CD Notes App
...
1. **Link GitHub Repository (Public)**
   - Berisi Notes App + `.github/workflows/ci.yml`.
   - Minimal ada 1 workflow run sukses di tab Actions.
2. **Screenshot SonarQube**
   - Tampilan hasil scan project kalian.
3. **Screenshot Output k6**
   - Bisa dari hasil run lokal (`k6 run loadtest/script.js`) atau dari GitHub Actions.
4. **Penjelasan singkat (1–2 paragraf)**
   - Insight atau pemahaman baru.
   - Tantangan saat membuat workflow & memperbaiki dependencies/lint.
   - Hal menarik atau penting yang ditemukan.
