...
Hasil scan bisa dilihat di `https://sonarcloud.io/project/overview?id=mrzacq_notes-app&organization=mrzacq`.
...
Keempat, setelah `sonar.organization` ditambah, scan malah gagal lagi dengan `Not authorized or project not found`. Ternyata project-nya belum pernah di-*import* ke SonarCloud — nggak cukup cuma nulis `sonar.projectKey` di properties, project itu harus ada dulu di dashboard SonarCloud (via **+ → Analyze new project**, pilih repo dari GitHub) supaya ke-bind ke GitHub App-nya (perlu buat PR decoration juga). SonarCloud juga auto-generate project key sendiri saat import (jadi `mrzacq_notes-app`, bukan `notes-app` polos), jadi `sonar.projectKey` di properties harus disamain persis dengan yang terdaftar di SonarCloud.

Kelima, setelah project key match dan auth lolos, muncul error baru: `You are running CI analysis while Automatic Analysis is enabled. Please consider disabling one or the other.` Ternyata SonarCloud otomatis mengaktifkan "Automatic Analysis" (scan otomatis dari sisi SonarCloud tanpa lewat CI) begitu sebuah repo GitHub diimport, dan itu konflik kalau ada CI job yang juga ngirim hasil analisis sendiri. Solusinya di dashboard project: **Administration → Analysis Method**, matiin toggle "Automatic Analysis" supaya cuma job `sonarqube_scan` di CI yang jadi satu-satunya sumber analisis. Insight-nya: integrasi third-party kayak gini sering punya "default behavior" tersembunyi (auto-scan) yang baru ketauan pas udah nyoba jalanin sendiri lewat CI.
